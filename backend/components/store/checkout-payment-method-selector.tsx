  ...
  return (
    <div className="space-y-2" role="radiogroup" aria-label="Payment method">
      ...
      <button
        type="button"
        role="radio"
        aria-checked={value === "payram"}
        onClick={() => onChange("payram")}
        className={`${base} ${value === "payram" ? active : idle}`}
      >
        <span
          className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 ${
            value === "payram" ? "border-[#b8962e] bg-[#b8962e]" : "border-neutral-300 bg-white"
          }`}
          aria-hidden
        >
