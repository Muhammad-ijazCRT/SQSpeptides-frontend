...
export class OrdersService {
  ...
  /** One DB round-trip for all line items (avoids N+1 sequential lookups on serverless DB). */
  private async resolveLineItemsForCreate(
    items: { productId: string; quantity: number }[]
  ): Promise<{ creates: { productId: string; quantity: number; price: Decimal }[]; subtotal: number }> {
    const keys = [...new Set(items.map((i) => String(i.productId).trim()).filter(Boolean))];
    if (keys.length === 0) {
      throw new BadRequestException("Cart has no valid line items.");
    }
    const t0 = Date.now();
    ...
    const resolve = (raw: string): (typeof products)[0] | null => {
      const key = raw.trim();
      return products.find((p) => p.id === key || p.slug === key) ?? null;
    };
    ...
    if (ms > 500) {
      this.log.warn(`Order line-item resolution slow: ${ms}ms for ${items.length} line(s), ${products.length} product row(s)`);
    }
    return { creates, subtotal: Math.round(subtotal * 100) / 100 };
  }
  ...
  async create(dto: CreateOrderDto, customerId?: string | null) {
    ...
    if ((payProvider === "nowpayments" || payProvider === "payram" || payProvider === "zelle") && (dto.storeCreditToUse ?? 0) > 0) {
      throw new BadRequestException(
        "Affiliate balance cannot be used with this payment method. Choose card payment or remove the balance amount."
      );
    }

    const { creates, subtotal } = await this.resolveLineItemsForCreate(dto.items);
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      const created = await tx.order.create({
        data: {
          ...
          paymentCompletion:
            payProvider === "nowpayments" || payProvider === "payram" || payProvider === "zelle"
              ? "awaiting_payment"
              : "paid",
          items: {
      ...
      if (payProvider !== "nowpayments" && payProvider !== "payram" && payProvider !== "zelle") {
    ...
    if (txMs > 1500) {
      this.log.warn(`Order create Prisma transaction slow: ${txMs}ms`);
    }
    ...
    if (totalMs > 2000) {
      this.log.warn(`Order create exceeded 2s: ${totalMs}ms (provider=${payProvider}, items=${dto.items.length})`);
    } else {
      this.log.debug(`Order create completed in ${totalMs}ms`);
    }
